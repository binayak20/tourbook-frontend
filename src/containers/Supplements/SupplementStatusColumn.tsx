...
export const SupplementStatusColumn: FC<Props> = ({ status, id }) => {
	const [isChecked, setChecked] = useState(status === 'Active');
	const { t } = useTranslation();
	const queryClient = useQueryClient();
	const { isAllowedTo } = useAccessContext();

	const { mutate, isLoading } = useMutation(() => supplementsAPI.updateStatus(id, !isChecked), {
		onSuccess: () => {
			setChecked((prev) => !prev);
			queryClient.invalidateQueries(['supplements']);
			message.success(t('Supplement status has been updated!'));
		},
		onError: (error: Error) => {
	...
	return (
		...
		>
			<Switch
				custom
				checked={isChecked}
				disabled={!isAllowedTo('CHANGE_SUPPLEMENT')}
